...
from .css_translator import HTMLTranslator, GenericTranslator
...
class SelectorList(list):
    ...
    def __getslice__(self, i, j):
        """
        deprecated but `list` builtin implements it only in Py2
        """
    ...
    def re(self, regex, index):
        """
        Call the ``.re()`` method for each element in this list and return
        their results flattened, as a list of unicode strings.
        """
        return TextListSelector([x.re(regex, index) for x in self])

    def wildcard(self, partten, index):
        return TextListSelector([x.wildcard(partten, index) for x in self])
    ...
    def __init__(self, text=None, type=None, namespaces=None, root=None,
        ...
        if text is not None:
            if not isinstance(text, six.text_type):
                raise TypeError(
                    "text argument should be of type %s" % six.text_type)
